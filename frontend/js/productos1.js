...
let listaProductos = [];
let idEditando = null; // Necesaria para saber si estamos en modo Edición o Creación

// --- CARGA DE PRODUCTOS ---
async function cargarProductosAdministracion() {
  try {
    const respuesta = await fetch("http://localhost:3000/administracion");
    listaProductos = await respuesta.json(); // Guardamos los datos en la variable global
...
// --- LÓGICA DE EDICIÓN (CLIC EN EL BOTÓN) ---
const tablaProductos = document.getElementById("tabla-productos");
tablaProductos.addEventListener("click", (e) => {
  if (e.target.classList.contains("btn-editar")) {
    const id = e.target.getAttribute("data-id");
    idEditando = id; // Activamos el modo edición

    const p = listaProductos.find((prod) => prod.id == id);

    // Rellenamos el formulario (Asegúrate que los 'name' del HTML coincidan)
    document.querySelector('input[name="nombre"]').value = p.nombre;
    document.querySelector('input[name="categoria"]').value = p.categoria;
...
formularioProductos.addEventListener("submit", async (e) => {
  ...
  const url = idEditando
    ? `http://localhost:3000/administracion/productos/${idEditando}`
    : "http://localhost:3000/administracion/productos";
  ...
  try {
    ...
    if (respuesta.ok) {
      alert(idEditando ? "Producto actualizado" : "Producto guardado");
      e.target.reset();
      idEditando = null; // Reiniciamos
      document.querySelector('button[type="submit"]').textContent =
        "Guardar Producto";
      await cargarProductosAdministracion();
    }
